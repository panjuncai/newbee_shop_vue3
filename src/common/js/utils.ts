...
// 改变tdist的声明方式，不需要添加复杂的类型
export const tdist: Record<string, string[]> = {
  210184: ['沈北新区', '210100'],
  210185: ['其它区', '210100'],
  420381: ['丹江口市', '420300'],
  420383: ['其它区', '420300'],
 ...
 * 获取一级地区列表
 */
export function getLev1Districts(): DistrictInfo[] {
 ...
 * @param code 一级地区代码
 */
export function getLev2Districts(code: string): DistrictInfo[] {
  if (code === '') {
    return []
  }
  ...
  const result: DistrictInfo[] = []
  for (let i = 1; i < 100; i++) {
    const n = code.substr(0, 2)
    const key = n + (i < 10 ? '0' + i + '00' : i + '00')
    const r = tdist[key]
 ...
 * @param code 二级地区代码
 */
export function getLev3Districts(code: string): DistrictInfo[] {
  if (code === '') {
    return []
  }
  ...
  const result: DistrictInfo[] = []
  for (let i = 1; i < 100; i++) {
    const n = code.substr(0, 4)
    const key = n + (i < 10 ? '0' + i : i)
    const r = tdist[key]
    if (typeof r !== 'undefined') {
      result.push({
        id: key,
